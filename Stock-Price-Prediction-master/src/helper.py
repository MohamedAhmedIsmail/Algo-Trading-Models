...
def denormalize(data, w0):

    for i in range(len(data)):
        data[i] = w0[i] * (data[i] + 1)
...
def load_data(filename, seq_len, norm_win):
    ...
    for i in range(len(data) - sequence_length):
        out.append(np.array(data[i: i + sequence_length], dtype=np.float))
        w0.append(float(out[i][0]))
    ...
    return [X_tr, Y_tr, np.array(X_te), np.array(Y_te), w0, out]
